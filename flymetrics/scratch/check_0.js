  ...
  let currentUser = {};
  try {
    currentUser = JSON.parse(localStorage.getItem('fm_user')) || {};
  } catch(e) {}
  ...
  window.saveTechProfile = function(e) {
    ...
    localStorage.setItem('fm_tech_nombre', nom);
    localStorage.setItem('fm_tech_tel', tel);
    localStorage.setItem('fm_tech_lic', lic);
    localStorage.setItem('fm_tech_est', est);
    ...
    if (elEst) elEst.innerHTML = `<span class="badge badge-active">${est} 🟢</span>`;

    showToast('🎉 ¡Perfil de Técnico Piloto actualizado exitosamente!', 'success');
  };
  ...
  function renderRecentAgendaTable() {
    ...
    tbody.innerHTML = recent.map(t => {
      ...
      return `
        <tr>
          <td style="font-weight:bold; color:var(--primary);">#FM-TRN-${t.id_turno}</td>
  ...
  function renderTechAgendaTable(turnos) {
    ...
    tbody.innerHTML = turnos.map(t => {
      ...
      if (t.estado === 'Pendiente') {
        actionBtn = `
          <button class="btn btn-primary btn-sm" onclick="submitDecision(${t.id_turno}, true)" style="margin-right:4px;">✓ Aceptar</button>
        ...
        actionBtn = `
          <button class="btn btn-primary btn-sm" onclick="submitCheckin(${t.id_turno})" style="margin-right:4px;">📍 Iniciar Check-in (En Proceso)</button>
          <button class="btn btn-danger btn-sm" onclick="openSolicitudModal(${t.id_turno}, 'Cancelacion')">📩 Solicitar Cancelación</button>
        `;
      ...
      return `
        <tr>
          <td>
            <a href="javascript:void(0)" onclick="openDetalleVueloModal(${t.id_turno})" style="font-weight:bold; color:var(--primary); text-decoration:underline;" title="Ver ficha técnica completa">#FM-TRN-${t.id_turno}</a>
          </td>
          <td>${t.fecha_de_turno || '—'}</td>
          <td>${hIni}</td>
          <td>${hFin}</td>
          ...
          <td>${badge}</td>
          <td>
            <button class="btn btn-ghost btn-sm" onclick="openDetalleVueloModal(${t.id_turno})" style="margin-right:4px;" title="Ver todos los datos del cliente y finca">🔍 Ver Ficha</button>
            ${actionBtn}
          </td>
        </tr>
      `;
  ...
  window.openDetalleVueloModal = async function(id_turno) {
    ...
    const idEl = document.getElementById('detTurnoId'); if (idEl) idEl.textContent = `#FM-TRN-${t.id_turno || t.id}`;
  ...
  window.submitEditClienteFromTech = async function(e) {
    ...
    try {
      ...
      showToast('✓ Datos de cliente actualizados exitosamente', 'success');
  ...
  window.submitDecision = async function(id_turno, decision) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Decisión registrada correctamente');
        await loadTechAgenda();
      } else {
  ...
  window.submitCheckin = async function(id_turno) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Check-in registrado. El servicio está En Proceso.');
        await loadTechAgenda();
      } else {
        const err = await res.json();
  ...
  window.submitCheckout = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Reporte de vuelo y enlace de Google Drive guardados exitosamente. Vuelo completado.');
        closeCheckoutModal();
        await loadTechAgenda();
      } else {
        const err = await res.json();
  ...
  function populateTurnosDropdowns() {
    ...
    const opts = _allTurnos.map(t => `<option value="${t.id_turno}">#FM-TRN-${t.id_turno} — ${t.servicio_nombre || 'Vuelo'} (${t.fecha_de_turno})</option>`).join('');
  ...
  window.submitEntregableModal = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Archivo entregable subido exitosamente');
  ...
  window.submitEntregableMain = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Entregable directo registrado correctamente');
        await loadTechAgenda();
      } else {
        const err = await res.json();
  ...
  window.openTechPagoModal = function() {
    ...
    if (sel) {
      const opts = _allTurnos.map(t => {
        ...
        return `<option value="${idT}" data-tarifa="${tarifa}" data-finca="${finca}" data-cliente="${cliente}" data-servicio="${servicio}">#FM-TRN-${idT} — ${servicio} | ${finca} (${cliente})</option>`;
      }).join('');
  ...
  window.onTechPagoTurnoChange = function() {
    ...
    if (opt && opt.value) {
      ...
      if (fincaInfoEl) {
        fincaInfoEl.innerHTML = `<span>🌾 <strong>Finca / Predio:</strong> <strong style="color:#1c82ad">${finca}</strong> | 👤 Cliente: <strong>${cliente}</strong> | ⚙️ ${servicio}</span>`;
      }
    } else {
      if (fincaInfoEl) {
        fincaInfoEl.innerHTML = '<span>🌾 <strong>Finca / Predio:</strong> Selecciona un vuelo para ver la finca asociada.</span>';
      }
    }
  };
  ...
  window.submitTechPago = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Recibo subido y enviado exitosamente a la Administración para desglose.');
        closeTechPagoModal();
        await loadTechAgenda();
      } else {
        const err = await res.json();
  ...
  window.openSolicitudModal = function(id_turno = null, tipo = 'Cancelacion') {
    ...
    if (sel) {
      sel.innerHTML = _allTurnos.map(t => `<option value="${t.id_turno}" ${t.id_turno == id_turno ? 'selected' : ''}>#FM-TRN-${t.id_turno} — ${t.servicio_nombre || 'Vuelo'} (${t.fecha_de_turno})</option>`).join('');
    }
  ...
  window.submitSolicitudTecnico = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        showToast('✓ Solicitud de autorización enviada al administrador exitosamente');
        closeSolicitudModal();
      } else {
        const err = await res.json();
  ...
  window.previewHeavyFileMain = function(e) {
    ...
    if (file.size > 15 * 1024 * 1024) {
      status2.textContent = '[REQUERIDA ⚡]';
      status2.style.color = 'var(--accent)';
    }
    if (file.size > 50 * 1024 * 1024) {
      status3.textContent = '[TURBO REQUERIDA ⚡]';
  ...
  window.startHeavyChunkedUploadMain = async function() {
    ...
    if (!file) {
      showToast('⚠ Selecciona un archivo pesado primero', 'warn');
      return;
    }
    ...
    status1.textContent = '[STREAMING 🟢]';
    if (useBox2) status2.textContent = '[STREAMING ⚡]';
    if (useBox3) status3.textContent = '[TURBO STREAM ⚡]';
    ...
    const timer = setInterval(async () => {
      ...
      document.getElementById('heavySpeedTextMain').textContent = `⚡ ${speed} MB/s (${workers} Cajas)`;
      document.getElementById('heavyMemoryTextMain').textContent = `Carga de Memoria: ${12 + workers * 6}% (Estable 🟢)`;

      if (processed >= totalChunks) {
        clearInterval(timer);
        status3.textContent = '[LIBERADA 🔒]';
        status2.textContent = '[LIBERADA 🔒]';
        ...
        try {
          ...
          if (res.ok) {
            showToast('✓ Archivo pesado procesado y almacenado correctamente con balanceo de Cajas Worker!');
            await loadTechAgenda();
          } else {
          ...
          btn.disabled = false;
          btn.textContent = '🚀 Iniciar Carga con Cajas Worker';
        }
      }
    }, 150);
  };
  ...
  window.calculateShiftViability = function(e) {
    ...
    if (exceeds) {
      ...
      warningAlert.innerHTML = `⚠️ El traslado toma ${travelHours} horas y excede el límite de salida de las 8:00 PM del piloto.`;
  ...
  window.togglePilotStatus = function(available) {
    ...
    showToast(`✓ Estado cambiado a ${available ? 'Disponible' : 'Ocupado en Vuelo'}`);
  };
